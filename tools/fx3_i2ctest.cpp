...
static void control_transfer_read(int devAddr, unsigned short address, int start, int len)
{
	int r = 0;
	int index = 0;
	int balance = len;
	...
	while ( len > 0 ) {
	      if(len>64) balance = 64;
	      else balance = len;
	      r = cyusb_control_transfer(h, 0xC0, 0xBB, devAddr, address, &buf[start+index], balance, 1000);
	      if ( r != balance ) {
	   	   printf("Error in control_transfer\n");
	      }
	      write(fd, &buf[start+index], balance);
	      address += balance ;
	      len -= balance;
	      index += balance;
	}
...
int main(int argc, char **argv)
{
	...
	if ( argv[2][0] == 'W' ) {
	   ...
	   if ( filsiz <= ( 64 * 1024 ) )
	      control_transfer_read(address, startAdr, 0, filsiz);
	   else  {
